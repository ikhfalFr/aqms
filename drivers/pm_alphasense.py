...
def update_sensor_value(sensor_reader_id, pin, sensor_value):
    try:
        cursor = db.cursor()
        cursor.execute("SELECT * FROM sensor_values WHERE sensor_reader_id = {} and pin = {}".format(sensor_reader_id, pin))
        isExist = cursor.fetchone()
        if isExist:
            cursor.execute("UPDATE sensor_values SET value = '{}' WHERE sensor_reader_id = {} and pin = {}".format(sensor_value, sensor_reader_id, pin))
        else:
            cursor.execute("INSERT INTO sensor_values(sensor_reader_id, pin, value) VALUES({}, {}, '{}')".format(sensor_reader_id, pin, sensor_value))
        db.commit()
        cursor.close()
...
def main():
    ...
    cursor.execute("SELECT sensor_code,baud_rate FROM sensor_readers WHERE id = {}".format(id))
    ...
    cursor.close()
    while True:
        instrument  = ModbusClient(method = 'rtu', port = result["sensor_code"], baudrate = int(result["baud_rate"]), timeout=1, parity='N', stopbits=1, bytesize=8)
        ...
        response = instrument.read_holding_registers(0, 9, unit=2)
        if response.isError():
